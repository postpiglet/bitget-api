...
class CandleInfo {
  ...
  public GetCurrentStochasticArray(gap:number) : any
  {
      return this.stochastic[this.stochastic.length-gap];
  }
  ...
  public GetStocReboundOpenPosition() : [POSITION_SIDE, Array<number>] 
  {
    ...
    {
      if(this.candleArray[this.candleArray.length-2][CANDLE_INDEX.CLOSE] > this.candleArray[this.candleArray.length-3][CANDLE_INDEX.CLOSE])
      {
  ...
  public GetRecentLowestCandle(period:number) : Array<number>
  {
    let index:number = this.candleArray.length-1
    let lowest:number = this.candleArray[this.candleArray.length-1][CANDLE_INDEX.CLOSE];
    for(var i = this.candleArray.length - period; i < this.candleArray.length; ++i)
    {
      if(lowest > this.candleArray[i][CANDLE_INDEX.LOW])
  ...
  public GetRecentHighestCandle(period:number) : Array<number>
  {
    let index:number = this.candleArray.length-1
    let highest:number = this.candleArray[this.candleArray.length-1][CANDLE_INDEX.CLOSE];
    for(var i = this.candleArray.length - period; i < this.candleArray.length; ++i)
  ...
  public IsHightVoulumeAverage(period:number, cur:number) : boolean
  {
    let accumAdd:number = 0;
    for(var i = this.candleArray.length - period; i < this.candleArray.length; ++i)
    {
      accumAdd += this.candleArray[i][CANDLE_INDEX.BVOLUME]
    }

    return this.candleArray[this.candleArray.length-cur][CANDLE_INDEX.BVOLUME] > (accumAdd/period)
  }
}
...
async function handleWsUpdateCandle(action:string, symbol:string, data, candleInfoDic:Map<string, CandleInfo>) {
  ...
  EnterPositionCheck(symbol);
}

async function EnterPositionCheck(symbol:string) {
  ...
  try
  {
    ...
    {
      orderInfo.reBoundCount = 0;
      switch(getStocOpenPosition[0])
      {
        case POSITION_SIDE.LONG:
          lock = true
          await openPosition(POSITION_SIDE.LONG, symbol, (1 / (openMaxCount - openAccumCount++)), false, candle5mInfo.GetRecentLowestCandle(14));
          lock = false
          break;
        case POSITION_SIDE.SHORT:
          lock = true
          await openPosition(POSITION_SIDE.SHORT, symbol, (1 / (openMaxCount - openAccumCount++)), false, candle5mInfo.GetRecentHighestCandle(14));
          lock = false
          break;
      }
    }
    ...
    {   
      ...
      if(orderInfo.isTrend)
      {
        ...
        {
          case POSITION_SIDE.LONG:
            if(candle5mInfo.GetCurrentCandleArray(1)[CANDLE_INDEX.CLOSE] < deadLineCandle[CANDLE_INDEX.CLOSE])
            {
              lock = true
              await closePosition(symbol, orderInfo);
              await WaitForOrderInfoReset(orderInfo);
      ...
      else
      {
        switch(orderInfo.side)
        {
          case POSITION_SIDE.LONG:
            ...
            else if(candle5mInfo.GetCurrentStochasticArray(1).k > 80)
            {
              lock = true
              await closePosition(symbol, orderInfo);
              lock = false
          ...
          case POSITION_SIDE.SHORT:
            ...
            else if(candle5mInfo.GetCurrentStochasticArray(1).k < 20)
            {
              lock = true
              await closePosition(symbol, orderInfo);
              lock = false
...
const openLeverageRatio:number = 0.2;
const openMaxCount:number = 10;
  ...
  try {
   ...
   // accumPersent = 0
    ...
    //for(var i = 0; i < 50; ++i)
    {
        ...
        //await wait(1000);
        // symbol = symbolRulesResult.data[i].symbol
        // let cc:CandleInfo|undefined = candle5mInfoDic.get(symbol)
        // if(cc!=undefined)
        // {
        //   await openPosition(POSITION_SIDE.LONG, symbol, (1 / (openMaxCount - openAccumCount++)), false, cc.GetCurrentCandleArray(0));
        //   await wait(1000);
        //   await closePosition(symbol, orderInfoDic.get(symbol));
        //   await WaitForOrderInfoReset(orderInfoDic.get(symbol));
        //   await openPosition(POSITION_SIDE.SHORT, symbol, (1 / (openMaxCount - openAccumCount++)), false, cc.GetCurrentCandleArray(0));
        // }
    }
