...
class CandleInfo {
  ...
  public IsClosing(side:POSITION_SIDE, gap:number) : boolean
  {
    ...
    {
      if(this.sma20Array[this.sma20Array.length-gap] < this.sma50Array[this.sma50Array.length-gap]
        && this.sma50Array[this.sma50Array.length-gap] < this.sma100Array[this.sma100Array.length-gap]
  ...
  public GetOpenPosion(gap:number) : POSITION_SIDE
  {
    ...
    if(this.sma20Array[this.sma20Array.length-gap] > this.sma50Array[this.sma50Array.length-gap]
      ...
      {
        // 20일선 기울기 상승
        if(this.sma20Array[this.sma20Array.length-gap] >= this.sma20Array[this.sma20Array.length-(gap+1)]
          && this.sma20Array[this.sma20Array.length-(gap+1)] >= this.sma20Array[this.sma20Array.length-(gap+2)]
          && this.sma20Array[this.sma20Array.length-(gap+2)] >= this.sma20Array[this.sma20Array.length-(gap+3)])
        {
          // stochastic 20이하 이면서 k가 d보다 위인 경우
          if(this.stochastic[this.stochastic.length-gap].d < 20
            && this.stochastic[this.stochastic.length-gap].k > this.stochastic[this.stochastic.length-gap].d)
            {
    ...
    // 하락추세(정배열)
    else if(this.sma20Array[this.sma20Array.length-gap] < this.sma50Array[this.sma50Array.length-gap]
      && this.sma50Array[this.sma50Array.length-gap] < this.sma100Array[this.sma100Array.length-gap]
      && this.sma100Array[this.sma100Array.length-gap] < this.sma200Array[this.sma200Array.length-gap])
      {
        // 20일선 기울기 하락
          ...
          && this.sma20Array[this.sma20Array.length-(gap+2)] <= this.sma20Array[this.sma20Array.length-(gap+3)])
        {
          ...
          if(this.stochastic[this.stochastic.length-gap].d > 80
            && this.stochastic[this.stochastic.length-gap].k < this.stochastic[this.stochastic.length-gap].d)
            {
...
const openLeverageRatio:number = 0.04;
const openMaxCount:number = 5;
